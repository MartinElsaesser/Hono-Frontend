...
async function fetcher<TEndpoint extends HonoEndpoint>([fn, args]: [TEndpoint, InferRequestType<TEndpoint>]): Promise<InferResponseType<TEndpoint>> {
  const res = await fn(args, undefined)
  return await res.json() as InferResponseType<TEndpoint>;
}

export function useHono<TEndpoint extends HonoEndpoint>(
  {endpoint, input}:
  { endpoint: TEndpoint, input: InferRequestType<TEndpoint>}
) {
  return useSWR(
    [endpoint, input],
    fetcher<TEndpoint>,
